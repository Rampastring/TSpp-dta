...
public:
    ...
    virtual void Set_Coord(const Coord& coord) override;
    ...
    virtual bool Is_In_Same_Zone(const Coord& cell) const override;
    ...
    virtual Coord Likely_Coord() const;
    ...
    virtual void Draw_Object(const ShapeSet* shapefile, int shapenum, Point2D& xy, Rect& rect, Dir256 rotation = DIR_N, int scale = 256, int a7 = 0, int a8 = 0, bool a9 = false, int a10 = 0, ShapeSet* z_shape = nullptr, int z_framenum = 0, Point2D z_off = Point2D(0, 0), ShapeFlags_Type flags = SHAPE_NORMAL) const;
